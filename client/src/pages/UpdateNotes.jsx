...
function UpdateNotes() {
  ...
  const [Notesimg,setiamge]=useState("")
  const [description, setdescription] = useState("")

  const handelSubmit=(e)=>{
    e.preventDefault()
    const notesData={title,body:description,Notesimg}
    axios.patch(`${import.meta.env.VITE_BASEURL}/notes/update/${notesId}`,notesData,{
  ...
  return (
    ...
    <form action="" className='position-relative w-50 m-auto p-4' style={{inset:"0px"}} onSubmit={handelSubmit}>
        ...
        <input type="text" className='mb-3 p-3 font' placeholder='image Url Here' style={{height:"50px"}} onChange={(e)=>setiamge(e.target.value)} />
